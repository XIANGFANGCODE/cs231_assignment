...
def cnn_fn_static(features, labels, mode, params):
    """
    ...
    network = tf.layers.conv2d(
        ...
        activation=tf.nn.relu)
    # Max pool layer
    network = tf.layers.max_pooling2d(inputs=network, pool_size=pool_size, strides=strides)
    ...
    network = tf.layers.dense(inputs=network, units=dense_size, activation=tf.nn.relu)
    network = tf.layers.dropout(
        inputs=network, rate=dropout_rate, training=mode == tf.estimator.ModeKeys.TRAIN)
    ...
    # [fc]
    scores = tf.layers.dense(inputs=network, units=class_num)
