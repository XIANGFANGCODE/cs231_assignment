...
def affine_forward(x, w, b):
    ...
    # out = x * w + b
    train_num = x.shape[0]
    train_vec = x.reshape(train_num, -1)
    out = np.dot(train_vec, w) + b
...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    train_num = x.shape[0]
    train_vec = x.reshape(train_num, -1) # N * D

    # out = w * x + b
    dw = np.dot(train_vec.T, dout)
    dtrain_vec = np.dot(dout, w.T)
    dx = dtrain_vec.reshape(*x.shape)
    # dout(N,M)  db(M,)
    ...
    for i in range(N):
        for j in range(F):
            ...
            for ho in range(HO):
                widx = 0
                for wo in range(WO):
                    out[i:i+1, j:j+1, ho:ho+1, wo:wo+1] = np.sum(x_pad[i:i+1, :, hidx:hidx+HH, widx:widx+WW] * w[j:j+1,:,:,:]) + b[j]
                    widx += stride
                hidx += stride
    ...
    for i in range(N):
        for j in range(F):
            ...
            for ho in range(HO):
                widx = 0
                for wo in range(WO):
                    dx_pad[i:i + 1, :, hidx:hidx + HH, widx:widx + WW] += w[j:j+1,:,:,:] * dout[i:i+1, j:j+1, ho:ho+1, wo:wo+1]
                    dw[j:j + 1, :, :, :] += x_pad[i:i + 1, :, hidx:hidx + HH, widx:widx + WW] * dout[i:i+1, j:j+1, ho:ho+1, wo:wo+1]
                    db[j] += dout[i:i+1, j:j+1, ho:ho+1, wo:wo+1]
                    widx += stride
                hidx += stride
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for j in range(C):
            for ho in range(HO):
                for wo in range(WO):
                    out[i:i+1, j:j+1, ho, wo] = np.max(x[i:i+1, j:j+1, hidx:hidx+pool_height, widx:widx+pool_width])
                    widx += stride
                hidx += stride
                widx = 0
            hidx = 0
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for ho in range(HO):
                for wo in range(WO):
                    xcopy = np.copy(x[i:i + 1, j:j + 1, hidx:hidx + pool_height, widx:widx + pool_width])
                    max_value = np.max(xcopy)
                    xcopy = xcopy - max_value + 1
                    xcopy[xcopy<1] = 0
                    dx[i:i+1, j:j+1, hidx:hidx+pool_height, widx:widx+pool_width] += xcopy * dout[i:i+1, j:j+1, ho, wo]
                    widx += stride
                hidx += stride
                widx = 0
            hidx = 0
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # def batchnorm_forward(x, gamma, beta, bn_param):
    N, C, H, W = x.shape
    x_copy = x.transpose(0, 2, 3, 1).reshape(-1, C)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    #def batchnorm_backward(dout, cache)
    N, C, H, W = dout.shape
    dout_copy = dout.transpose(0, 2, 3, 1).reshape(-1, C)
